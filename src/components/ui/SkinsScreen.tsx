...
export default function SkinsScreen({ onBack }: SkinsScreenProps) {
  ...
  const [selected, setSelected] = useState<SkinId>(user?.skin ?? 'default')

  const handleSelect = (skin: SkinConfig) => {
    if (!user?.unlockedSkins.includes(skin.id)) return
    setSelected(skin.id)
    syncProgress({ skin: skin.id, unlockedSkins: user.unlockedSkins })
  }

  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-dark-900 px-4 py-12">
      <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at top, rgba(128,0,255,0.04) 0%, transparent 60%)' }} />
      ...
      <div className="relative z-10 w-full max-w-2xl flex flex-col gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-xs text-dark-300 ml-2">
            {user?.unlockedSkins.length ?? 1}/{SKINS.length} desbloqueadas
          </span>
        </div>

        <div className="text-dark-300 text-xs font-mono -mt-2">
          LVL atual: <span className="text-neon-cyan">{user?.level ?? 1}</span> — Acumule XP para desbloquear skins de nível maior
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {SKINS.map((skin) => {
            const unlocked = user?.unlockedSkins.includes(skin.id) ?? skin.id === 'default'
            ...
            return (
              <button
                ...
                style={{
                  borderColor: isSelected ? skin.glowColor+'80' : unlocked ? skin.glowColor+'30' : '#0d1a24',
                  background: isSelected ? skin.glowColor+'12' : unlocked ? skin.glowColor+'06' : 'rgba(5,12,18,0.6)',
                  boxShadow: isSelected ? `0 0 20px ${skin.glowColor}30` : 'none',
                  opacity: unlocked ? 1 : 0.4,
                }}
              >
                {isSelected && <div className="absolute top-2 right-2" style={{ color: skin.glowColor }}><IconCheck size={12} /></div>}
                {!unlocked && <div className="absolute top-2 right-2 text-dark-300"><IconLock size={12} /></div>}
                <SnakePreview skin={skin} />
                <div className="flex flex-col items-center gap-0.5">
                  <div className="font-mono text-xs font-bold" style={{ color: unlocked ? skin.glowColor : '#4a6070' }}>{skin.name}</div>
                  {!unlocked && <div className="text-[9px] font-mono text-dark-300">LVL {skin.requiredLevel}</div>}
                  {unlocked && isSelected && <div className="text-[9px] font-mono" style={{ color: skin.glowColor+'80' }}>EQUIPADA</div>}
                </div>
              </button>
            )
          })}
...
function SnakePreview({ skin }: { skin: SkinConfig }) {
  ...
  useEffect(() => {
    ...
    const size = 10
    const segments = [
      {x:4,y:2},{x:3,y:2},{x:2,y:2},{x:2,y:3},{x:2,y:4},{x:3,y:4},{x:4,y:4},
    ]
    const draw = () => {
      tickRef.current++
      ...
      segments.forEach((seg, idx) => {
        ...
        if (skin.pattern === 'rainbow') {
          color = `hsl(${(t*2+idx*25)%360},100%,60%)`
        } else if (skin.pattern === 'gradient') {
          const r1=parseInt(skin.headColor.slice(1,3),16), g1=parseInt(skin.headColor.slice(3,5),16), b1=parseInt(skin.headColor.slice(5,7),16)
          const r2=parseInt(skin.bodyColor.slice(1,3),16), g2=parseInt(skin.bodyColor.slice(3,5),16), b2=parseInt(skin.bodyColor.slice(5,7),16)
  ...
  return <canvas ref={canvasRef} width={70} height={70} className="rounded" />
}
