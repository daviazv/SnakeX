...
import {
  IconTrophy, IconArrowLeft, IconMedal, IconCrown,
  IconChartBar, IconGhost, IconRefresh, IconWorld,
...
export default function LeaderboardScreen({ onBack }: LeaderboardScreenProps) {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-dark-900 px-4 py-12">
      <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at top, rgba(255,238,0,0.03) 0%, transparent 60%)' }} />
      ...
      <div className="relative z-10 w-full max-w-lg flex flex-col gap-6">
        <div className="flex items-center gap-4">
          ...
          <button onClick={load} className="flex items-center gap-1.5 text-dark-300 hover:text-neon-green transition-colors text-xs font-mono">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 text-[10px] font-mono text-dark-300 ml-1">
            <IconWorld size={11} /> MongoDB
          </div>
        </div>

        {loading && (
          <div className="flex items-center justify-center gap-3 py-16 text-dark-400 font-mono text-xs">
            {[0,1,2].map(i => (
        ...
        {error && (
          <div className="text-center py-8 font-mono text-xs text-neon-pink">{error}</div>
        )}
            ...
            {entries.map((entry) => {
              ...
              return (
                <div
                  ...
                  style={{
                    borderColor: isMe ? '#00ff8840' : entry.rank <= 3 ? `${color}30` : '#0d1a2420',
                    background: isMe ? 'rgba(0,255,136,0.06)' : entry.rank <= 3 ? `${color}08` : 'rgba(5,12,18,0.6)',
                    boxShadow: isMe ? '0 0 15px rgba(0,255,136,0.1)' : 'none',
                  }}
                >
        ...
        <p className="text-center text-[10px] font-mono text-dark-400">
          Rankings reais de todos os jogadores — persistidos no MongoDB Atlas
        </p>
      </div>
    </div>
  )
